...
#define GTK_HEATHKIT_KNOB_GET_PRIVATE(obj) (G_TYPE_INSTANCE_GET_PRIVATE((obj), GTK_TYPE_HEATHKIT_KNOB, GtkHeathkitKnobPrivate))

G_DEFINE_TYPE(GtkHeathkitKnob, gtk_knob, GTK_TYPE_RANGE);

typedef struct _GtkHeathkitKnobPrivate GtkHeathkitKnobPrivate;

struct _GtkHeathkitKnobPrivate
{
    int knob_x;
    int knob_y;
    int knob_step;
...
static void knob_expose(GtkWidget *widget, int knob_x, int knob_y, GdkEventExpose *event, int arc_offset)
{
    /** check resize **/
    int grow;
    GtkHeathkitKnobPrivate *priv = GTK_HEATHKIT_KNOB_GET_PRIVATE(widget);
    ...
    if (priv->model != 3) {
        ...
        GtkHeathkitKnob *knob = GTK_HEATHKIT_KNOB(widget);
...
{
    g_assert(GTK_IS_HEATHKIT_KNOB(widget));
    GtkHeathkitKnob *knob = GTK_HEATHKIT_KNOB(widget);
    GtkHeathkitKnobPrivate *priv = GTK_HEATHKIT_KNOB_GET_PRIVATE(knob);
...
{
    GtkHeathkitKnob *knob = GTK_HEATHKIT_KNOB(object);
...
/****************************************************************
 ** init the GtkHeathkitKnobClass
 */

static void gtk_knob_class_init (GtkHeathkitKnobClass *klass)
{
...
//--------- connect the events with funktions
    ...
    g_type_class_add_private(obj_class, sizeof (GtkHeathkitKnobPrivate));
}
...
static void gtk_knob_init (GtkHeathkitKnob *knob)
{
...
{
    GtkWidget *widget = GTK_WIDGET( g_object_new (GTK_TYPE_HEATHKIT_KNOB, NULL ));
